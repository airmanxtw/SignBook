...
module PlaywrightProvider =
    ...
    let start (saveLoginInfo : (LoginInfo) -> Result<unit,exn>) (model:Option<LoginInfo>)  =
        Result.protect(fun()->
            ...
            let mutable ocrTxt =""
            AnsiConsole.Status().Start("正在載入...", fun ctx ->
            ...
            AnsiConsole.Status().Start("登入中...", fun ctx ->
            ...
            if not (String.IsNullOrEmpty alertMessage) then
                ...
                let selectBtns = page.QuerySelectorAllAsync "#ctl00_ContentPlaceHolder1_gv_signbook input[type='submit']:not([disabled])" 
                                |> Async.AwaitTask 
                                |> Async.RunSynchronously
                                |> fun buttons ->
                                    buttons
                                    ...
                                    |> Seq.map (fun button ->
                                        let id    = button.GetAttributeAsync "id"    |> Async.AwaitTask |> Async.RunSynchronously
                                        let value = button.GetAttributeAsync "value" |> Async.AwaitTask |> Async.RunSynchronously
                                        {id = id; value = value}
                                    )
                                    |> Seq.toList
                ...
                if selectBtns.Length = 0 then
                    ...
                    let selectBtn = AnsiConsole.Prompt(SelectionPrompt<SelectBtn>().Title("選擇要簽到的項目:")
                                                        .PageSize(10)
                                                        .UseConverter(fun btn -> $"{btn.value}")
                                                        .AddChoices selectBtns
                                                        )

                    AnsiConsole.Status().Start("簽到中...", fun ctx ->                    
                    ...
                    if not (String.IsNullOrEmpty alertMessage) then
                        ...
                        page.QuerySelectorAsync $"xpath=//input[@id='{selectBtn.id}']/parent::td/following-sibling::td[3]/span"
                        |> Async.AwaitTask 
                        |> Async.RunSynchronously
                        |> fun element ->
